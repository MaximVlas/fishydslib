...
- [8) Notes on Enums and Constants](#8-notes-on-enums-and-constants)
...
|-----|------|-------------|
| `dc_client_t` | Opaque struct | Main client handle created by `dc_client_create` and owned/freed by `dc_client_free`. |
| `dc_gateway_info_t` | Struct | Result model for `/gateway/bot` metadata (`url`, shard recommendation, session start limits). |
| `dc_client_config_t` | Struct | Client startup configuration passed into `dc_client_create` / `dc_client_init`. |

#### `dc_client_config_t` Fields

| Field | Type | Description |
|-------|------|-------------|
| `token` | `const char*` | Bot or bearer token string. |
| `auth_type` | `dc_http_auth_type_t` | Authorization scheme (`Bot`/`Bearer`). |
| `intents` | `uint32_t` | Gateway intents bitmask. |
| `shard_id`, `shard_count` | `uint32_t` | Optional shard coordinates. |
| `large_threshold` | `uint32_t` | Identify `large_threshold` (50-250, `0` to omit). |
| `user_agent` | `const char*` | Explicit User-Agent string override. |
| `user_agent_info` | `dc_user_agent_t` | Structured User-Agent descriptor. |
| `use_user_agent_info` | `int` | Use `user_agent_info` when `user_agent` is not provided. |
| `event_callback` | `dc_gateway_event_callback_t` | Gateway dispatch callback. |
| `state_callback` | `dc_gateway_state_callback_t` | Gateway connection state callback. |
| `user_data` | `void*` | User pointer passed to callbacks. |
| `http_timeout_ms` | `uint32_t` | HTTP timeout in milliseconds. |
| `gateway_timeout_ms` | `uint32_t` | Gateway wait timeout in milliseconds. |
| `enable_compression` | `int` | Enable gateway compression. |
| `enable_payload_compression` | `int` | Enable Identify payload compression. |
| `log_callback` | `dc_log_callback_t` | Optional runtime logging callback. |
| `log_user_data` | `void*` | User pointer for `log_callback`. |
| `log_level` | `dc_log_level_t` | Runtime log filter level. |
...
| `dc_client_modify_current_user_json(dc_client_t* client, const char* json_body, dc_user_t* user)` | `client`: Discord client, `json_body`: JSON payload for `PATCH /users/@me`, `user`: Output updated user (optional) | `dc_status_t`: `DC_OK` on success, error code on failure | Modify current user account fields |
| `dc_client_get_current_user_guilds_json(dc_client_t* client, uint32_t limit, dc_snowflake_t before, dc_snowflake_t after, int with_counts, dc_string_t* guilds_json)` | `client`: Discord client, `limit`: Max guilds (1-200, 0 default), `before`: Guild cursor (0 to omit), `after`: Guild cursor (0 to omit), `with_counts`: Include approximate counts, `guilds_json`: Output guild list JSON (overwritten) | `dc_status_t`: `DC_OK` on success, error code on failure | List current user guilds (`GET /users/@me/guilds`) |
| `dc_client_get_current_user_guild_member_json(dc_client_t* client, dc_snowflake_t guild_id, dc_string_t* member_json)` | `client`: Discord client, `guild_id`: Guild ID, `member_json`: Output member JSON (overwritten) | `dc_status_t`: `DC_OK` on success, error code on failure | Get current user member object for a guild (`GET /users/@me/guilds/{guild.id}/member`) |
| `dc_client_get_current_user_guild_member(dc_client_t* client, dc_snowflake_t guild_id, dc_guild_member_t* member)` | `client`: Discord client, `guild_id`: Guild ID, `member`: Output typed member (overwritten) | `dc_status_t`: `DC_OK` on success, error code on failure | Typed wrapper for current user guild member |
| `dc_client_leave_guild(dc_client_t* client, dc_snowflake_t guild_id)` | `client`: Discord client, `guild_id`: Guild ID | `dc_status_t`: `DC_OK` on success, error code on failure | Leave guild (`DELETE /users/@me/guilds/{guild.id}`) |
| `dc_client_create_dm_channel_json(dc_client_t* client, const char* json_body, dc_channel_t* channel)` | `client`: Discord client, `json_body`: JSON payload for `POST /users/@me/channels`, `channel`: Output channel (optional) | `dc_status_t`: `DC_OK` on success, error code on failure | Create DM/group DM channel with raw JSON |
| `dc_client_create_dm_channel(dc_client_t* client, dc_snowflake_t recipient_id, dc_channel_t* channel)` | `client`: Discord client, `recipient_id`: Recipient user ID, `channel`: Output channel (optional) | `dc_status_t`: `DC_OK` on success, error code on failure | Convenience helper to create a DM channel from a recipient ID |
| `dc_client_get_current_user_connections_json(dc_client_t* client, dc_string_t* connections_json)` | `client`: Discord client, `connections_json`: Output connections JSON (overwritten) | `dc_status_t`: `DC_OK` on success, error code on failure | Get connected accounts (`GET /users/@me/connections`) |
| `dc_client_get_current_user_application_role_connection_json(dc_client_t* client, dc_snowflake_t application_id, dc_string_t* role_connection_json)` | `client`: Discord client, `application_id`: Application ID, `role_connection_json`: Output JSON (overwritten) | `dc_status_t`: `DC_OK` on success, error code on failure | Get application role connection (`GET /users/@me/applications/{application.id}/role-connection`) |
| `dc_client_update_current_user_application_role_connection_json(dc_client_t* client, dc_snowflake_t application_id, const char* json_body, dc_string_t* role_connection_json)` | `client`: Discord client, `application_id`: Application ID, `json_body`: PUT body, `role_connection_json`: Output JSON (overwritten) | `dc_status_t`: `DC_OK` on success, error code on failure | Update application role connection (`PUT /users/@me/applications/{application.id}/role-connection`) |
...
| `dc_client_search_guild_messages_json(dc_client_t* client, dc_snowflake_t guild_id, const char* encoded_query, dc_string_t* search_json)` | `client`: Discord client, `guild_id`: Guild ID, `encoded_query`: URL-encoded query string without leading `?` (NULL/empty to omit), `search_json`: Output JSON string (overwritten) | `dc_status_t`: `DC_OK` on success, error code on failure | Fetch the raw search response from `GET /guilds/{guild.id}/messages/search` |
...
| `dc_client_interaction_get_original_response_json(dc_client_t* client, dc_snowflake_t application_id, const char* interaction_token, dc_snowflake_t thread_id, dc_string_t* message_json)` | `client`: Discord client, `application_id`: Application ID, `interaction_token`: Interaction token, `thread_id`: Optional thread ID (0 to omit), `message_json`: Output message JSON | `dc_status_t`: `DC_OK` on success, error code on failure | Fetch the original interaction response message as raw JSON |
...
| `dc_client_interaction_get_followup_message_json(dc_client_t* client, dc_snowflake_t application_id, const char* interaction_token, dc_snowflake_t message_id, dc_snowflake_t thread_id, dc_string_t* message_json)` | `client`: Discord client, `application_id`: Application ID, `interaction_token`: Interaction token, `message_id`: Follow-up message ID, `thread_id`: Optional thread ID (0 to omit), `message_json`: Output message JSON | `dc_status_t`: `DC_OK` on success, error code on failure | Fetch a follow-up interaction message as raw JSON |
...
#### `dc_interaction_callback_type_t` Values

| Enum Constant | Value | Meaning |
|---------------|-------|---------|
| `DC_INTERACTION_CALLBACK_PONG` | `1` | ACK for Ping interactions. |
| `DC_INTERACTION_CALLBACK_CHANNEL_MESSAGE_WITH_SOURCE` | `4` | Immediate message response with source. |
| `DC_INTERACTION_CALLBACK_DEFERRED_CHANNEL_MESSAGE_WITH_SOURCE` | `5` | Deferred message response with source. |
| `DC_INTERACTION_CALLBACK_DEFERRED_UPDATE_MESSAGE` | `6` | Deferred message component update. |
| `DC_INTERACTION_CALLBACK_UPDATE_MESSAGE` | `7` | Edit the source message for a component interaction. |
| `DC_INTERACTION_CALLBACK_APPLICATION_COMMAND_AUTOCOMPLETE_RESULT` | `8` | Autocomplete result payload. |
| `DC_INTERACTION_CALLBACK_MODAL` | `9` | Show modal dialog response. |
| `DC_INTERACTION_CALLBACK_LAUNCH_ACTIVITY` | `12` | Launch embedded activity callback. |
...
| `dc_client_search_guild_messages_json` | `GET /guilds/{guild.id}/messages/search` (caller supplies encoded query string) |
...
| `dc_client_get_guild_audit_log_json` | `GET /guilds/{guild.id}/audit-logs` (supports `user_id/action_type/before/after/limit`) |
| `dc_client_get_guild_widget_settings_json` | `GET /guilds/{guild.id}/widget` |
| `dc_client_modify_guild_widget_json` | `PATCH /guilds/{guild.id}/widget` |
| `dc_client_get_guild_widget_json` | `GET /guilds/{guild.id}/widget.json` |
| `dc_client_get_guild_vanity_url_json` | `GET /guilds/{guild.id}/vanity-url` |
| `dc_client_get_guild_welcome_screen_json` | `GET /guilds/{guild.id}/welcome-screen` |
| `dc_client_modify_guild_welcome_screen_json` | `PATCH /guilds/{guild.id}/welcome-screen` |
| `dc_client_get_guild_onboarding_json` | `GET /guilds/{guild.id}/onboarding` |
| `dc_client_modify_guild_onboarding_json` | `PUT /guilds/{guild.id}/onboarding` |
| `dc_client_modify_guild_incident_actions_json` | `PUT /guilds/{guild.id}/incident-actions` |
| `dc_client_list_auto_moderation_rules_json` | `GET /guilds/{guild.id}/auto-moderation/rules` |
| `dc_client_get_auto_moderation_rule_json` | `GET /guilds/{guild.id}/auto-moderation/rules/{auto_moderation_rule.id}` |
| `dc_client_create_auto_moderation_rule_json` | `POST /guilds/{guild.id}/auto-moderation/rules` |
| `dc_client_modify_auto_moderation_rule_json` | `PATCH /guilds/{guild.id}/auto-moderation/rules/{auto_moderation_rule.id}` |
| `dc_client_delete_auto_moderation_rule` | `DELETE /guilds/{guild.id}/auto-moderation/rules/{auto_moderation_rule.id}` |
| `dc_client_create_stage_instance_json` | `POST /stage-instances` |
| `dc_client_get_stage_instance_json` | `GET /stage-instances/{channel.id}` |
| `dc_client_modify_stage_instance_json` | `PATCH /stage-instances/{channel.id}` |
| `dc_client_delete_stage_instance` | `DELETE /stage-instances/{channel.id}` |
| `dc_client_get_poll_answer_voters_json` | `GET /channels/{channel.id}/polls/{message.id}/answers/{answer_id}` (supports `after/limit`) |
| `dc_client_end_poll_json` | `POST /channels/{channel.id}/polls/{message.id}/expire` |
| `dc_client_send_soundboard_sound_json` | `POST /channels/{channel.id}/send-soundboard-sound` |
| `dc_client_list_default_soundboard_sounds_json` | `GET /soundboard-default-sounds` |
| `dc_client_list_guild_soundboard_sounds_json` | `GET /guilds/{guild.id}/soundboard-sounds` |
| `dc_client_get_guild_soundboard_sound_json` | `GET /guilds/{guild.id}/soundboard-sounds/{sound.id}` |
| `dc_client_create_guild_soundboard_sound_json` | `POST /guilds/{guild.id}/soundboard-sounds` |
| `dc_client_modify_guild_soundboard_sound_json` | `PATCH /guilds/{guild.id}/soundboard-sounds/{sound.id}` |
| `dc_client_delete_guild_soundboard_sound` | `DELETE /guilds/{guild.id}/soundboard-sounds/{sound.id}` |
| `dc_client_list_voice_regions_json` | `GET /voice/regions` |
| `dc_client_get_current_user_voice_state_json` | `GET /guilds/{guild.id}/voice-states/@me` |
| `dc_client_get_user_voice_state_json` | `GET /guilds/{guild.id}/voice-states/{user.id}` |
| `dc_client_modify_current_user_voice_state_json` | `PATCH /guilds/{guild.id}/voice-states/@me` |
| `dc_client_modify_user_voice_state_json` | `PATCH /guilds/{guild.id}/voice-states/{user.id}` |
...
| Function | Parameters | Return Value | Description |
|----------|------------|--------------|-------------|
| `dc_alloc_set_hooks(const dc_alloc_hooks_t* hooks)` | `hooks`: Custom allocation hooks to install | `dc_status_t`: `DC_OK` on success, error code on failure | Install custom alloc/realloc/free hooks |
| `dc_alloc_get_hooks(dc_alloc_hooks_t* hooks)` | `hooks`: Pointer to store current hooks | `dc_status_t`: `DC_OK` on success, error code on failure | Retrieve currently active memory hooks |
...
| `dc_alloc_aligned(size_t size, size_t alignment)` | `size`: Size in bytes to allocate, `alignment`: Alignment requirement in bytes | `void*`: Pointer to aligned allocated memory, or NULL on failure | Allocate aligned block (default libc hooks only) |
...
| `dc_gateway_event_parse_channel(const char* event_data, dc_channel_t* channel)` | `event_data`: Channel event JSON data, `channel`: Output channel struct to populate | `dc_status_t`: `DC_OK` on success, error code on failure | Parse channel payload (`CHANNEL_CREATE/UPDATE/DELETE`) |
...
| `dc_gateway_event_parse_guild_update(const char* event_data, dc_guild_t* guild)` | `event_data`: GUILD_UPDATE JSON data, `guild`: Output guild model to populate | `dc_status_t`: `DC_OK` on success, error code on failure | Parse `GUILD_UPDATE` payload |
| `dc_gateway_guild_delete_init(dc_gateway_guild_delete_t* guild_delete)` | `guild_delete`: GUILD_DELETE model to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Init `GUILD_DELETE` model |
| `dc_gateway_guild_delete_free(dc_gateway_guild_delete_t* guild_delete)` | `guild_delete`: GUILD_DELETE model to free | `void` | Free `GUILD_DELETE` model |
| `dc_gateway_event_parse_guild_delete(const char* event_data, dc_gateway_guild_delete_t* guild_delete)` | `event_data`: GUILD_DELETE JSON data, `guild_delete`: Output model to populate | `dc_status_t`: `DC_OK` on success, error code on failure | Parse `GUILD_DELETE` payload while preserving whether `unavailable` was present |
...
| `dc_gateway_message_update_init(dc_gateway_message_update_t* update)` | `update`: MESSAGE_UPDATE wrapper to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Init partial `MESSAGE_UPDATE` wrapper |
| `dc_gateway_message_update_free(dc_gateway_message_update_t* update)` | `update`: MESSAGE_UPDATE wrapper to free | `void` | Free partial `MESSAGE_UPDATE` wrapper |
| `dc_gateway_event_parse_message_update(const char* event_data, dc_gateway_message_update_t* update)` | `event_data`: MESSAGE_UPDATE JSON data, `update`: Output wrapper to populate | `dc_status_t`: `DC_OK` on success, error code on failure | Parse partial `MESSAGE_UPDATE` payload with typed common fields plus `raw_json` for unlifted fields |
| `dc_gateway_message_delete_init(dc_gateway_message_delete_t* message_delete)` | `message_delete`: MESSAGE_DELETE wrapper to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Init `MESSAGE_DELETE` wrapper |
| `dc_gateway_message_delete_free(dc_gateway_message_delete_t* message_delete)` | `message_delete`: MESSAGE_DELETE wrapper to free | `void` | Free `MESSAGE_DELETE` wrapper |
| `dc_gateway_event_parse_message_delete(const char* event_data, dc_gateway_message_delete_t* message_delete)` | `event_data`: MESSAGE_DELETE JSON data, `message_delete`: Output wrapper to populate | `dc_status_t`: `DC_OK` on success, error code on failure | Parse `MESSAGE_DELETE` payload |
| `dc_gateway_message_delete_bulk_init(dc_gateway_message_delete_bulk_t* bulk_delete)` | `bulk_delete`: MESSAGE_DELETE_BULK wrapper to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Init `MESSAGE_DELETE_BULK` wrapper |
| `dc_gateway_message_delete_bulk_free(dc_gateway_message_delete_bulk_t* bulk_delete)` | `bulk_delete`: MESSAGE_DELETE_BULK wrapper to free | `void` | Free `MESSAGE_DELETE_BULK` wrapper |
| `dc_gateway_event_parse_message_delete_bulk(const char* event_data, dc_gateway_message_delete_bulk_t* bulk_delete)` | `event_data`: MESSAGE_DELETE_BULK JSON data, `bulk_delete`: Output wrapper to populate | `dc_status_t`: `DC_OK` on success, error code on failure | Parse `MESSAGE_DELETE_BULK` payload |
| `dc_gateway_event_parse_interaction_create(const char* event_data, dc_interaction_t* interaction)` | `event_data`: INTERACTION_CREATE JSON data, `interaction`: Output interaction model to populate | `dc_status_t`: `DC_OK` on success, error code on failure | Parse typed `INTERACTION_CREATE` payload (command/component/modal with raw JSON capture for complex sub-objects) |
...
| `dc_avatar_decoration_data_init(dc_avatar_decoration_data_t* data)` | `data`: Avatar-decoration data struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize nested avatar-decoration data object |
| `dc_avatar_decoration_data_free(dc_avatar_decoration_data_t* data)` | `data`: Avatar-decoration data struct to free | `void` | Free nested avatar-decoration data fields |
| `dc_nameplate_data_init(dc_nameplate_data_t* data)` | `data`: Nameplate data struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize user nameplate data object |
| `dc_nameplate_data_free(dc_nameplate_data_t* data)` | `data`: Nameplate data struct to free | `void` | Free user nameplate data fields |
| `dc_collectibles_init(dc_collectibles_t* data)` | `data`: Collectibles struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize user collectibles container |
| `dc_collectibles_free(dc_collectibles_t* data)` | `data`: Collectibles struct to free | `void` | Free collectibles nested fields |
| `dc_user_primary_guild_init(dc_user_primary_guild_t* data)` | `data`: Primary-guild struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize user primary-guild object |
| `dc_user_primary_guild_free(dc_user_primary_guild_t* data)` | `data`: Primary-guild struct to free | `void` | Free user primary-guild fields |
...
**Notes**
- `dc_guild_t.features` now captures guild features as a typed `dc_vec_t` of `dc_string_t` (guarded by `has_features`).
- `dc_guild_t.roles` now captures typed role objects as `dc_role_list_t` (guarded by `has_roles`).
- `dc_guild_t.emojis`, `dc_guild_t.welcome_screen`, `dc_guild_t.stickers`, and `dc_guild_t.incidents_data` are now parsed into dedicated typed structs.
- Compatibility raw-JSON mirrors are still retained (`roles_json`, `emojis_json`, `welcome_screen_json`, `stickers_json`, `incidents_data_json`) for transitional/debug workflows.
...
**Notes**
- `dc_guild_member_t.banner` is now supported as a nullable per-guild member banner field.
- **Remaining typed gap:** guild-member `avatar_decoration_data` is not yet represented as a dedicated typed field in `dc_guild_member_t`.
...
**Notes**
- `dc_role_t.description` is supported as a nullable role-description field and is included in JSON parse/serialize paths.
...
**Notes**
- `dc_component_type_t` now includes modal component types `DC_COMPONENT_TYPE_RADIO_GROUP` (`21`), `DC_COMPONENT_TYPE_CHECKBOX_GROUP` (`22`), and `DC_COMPONENT_TYPE_CHECKBOX` (`23`).
- `dc_component_t.default_val` stores the top-level checkbox `default` field.
- `dc_component_t.value` stores string-valued single-value component responses (for example, radio groups), and `dc_component_t.value_bool` stores checkbox boolean responses.
- `dc_component_t.values` stores string-valued interaction response arrays (for example, string selects and checkbox groups) when `has_values` is set.
- `dc_component_t.snowflake_values` stores snowflake-valued interaction response arrays (user/role/mentionable/channel selects and file uploads) when `has_snowflake_values` is set.
- `dc_component_t.uses_component_type` preserves the Discord response shape difference between message interaction responses (`component_type`) and modal responses / create payloads (`type`).
...
**Notes**
- `dc_presence_t` now captures raw `activities` and `client_status` payload sections via `activities_json` / `client_status_json` with `has_activities` / `has_client_status`.
- **Current limitation:** activities and client-status are not yet parsed into dedicated typed sub-structures.
...
| `dc_reaction_init(dc_reaction_t* reaction)` | `reaction`: Reaction struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize reaction object used in message `reactions` |
| `dc_reaction_free(dc_reaction_t* reaction)` | `reaction`: Reaction struct to free | `void` | Free reaction-owned fields |
| `dc_sticker_item_init(dc_sticker_item_t* item)` | `item`: Sticker-item struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize sticker-item object |
| `dc_sticker_item_free(dc_sticker_item_t* item)` | `item`: Sticker-item struct to free | `void` | Free sticker-item fields |
| `dc_channel_mention_init(dc_channel_mention_t* mention)` | `mention`: Channel-mention struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize channel mention object for crossposted messages |
| `dc_channel_mention_free(dc_channel_mention_t* mention)` | `mention`: Channel-mention struct to free | `void` | Free channel mention fields |
| `dc_role_subscription_data_init(dc_role_subscription_data_t* data)` | `data`: Role-subscription data struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize role-subscription payload object |
| `dc_role_subscription_data_free(dc_role_subscription_data_t* data)` | `data`: Role-subscription data struct to free | `void` | Free role-subscription payload fields |
| `dc_message_call_init(dc_message_call_t* call)` | `call`: Message-call struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize call metadata nested in message objects |
| `dc_message_call_free(dc_message_call_t* call)` | `call`: Message-call struct to free | `void` | Free call metadata fields |
| `dc_message_activity_init(dc_message_activity_t* activity)` | `activity`: Message-activity struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize rich-presence activity metadata |
| `dc_message_activity_free(dc_message_activity_t* activity)` | `activity`: Message-activity struct to free | `void` | Free message-activity fields |
| `dc_message_reference_init(dc_message_reference_t* ref)` | `ref`: Message-reference struct to initialize | `dc_status_t`: `DC_OK` on success, error code on failure | Initialize message reference payload |
| `dc_message_reference_free(dc_message_reference_t* ref)` | `ref`: Message-reference struct to free | `void` | Free message-reference fields |
