...
int main(void) {
    ...
    // Resolve token from process env first, then fallback dotenv files.
    const char* dotenv_paths[] = {"./.env", "./.env.ex"};
    st = dc_env_get_discord_token(dotenv_paths,
                                  sizeof(dotenv_paths) / sizeof(dotenv_paths[0]),
                                  DC_ENV_FLAG_REQUIRE_PRIVATE_FILE,
                                  &token);
    ...
    (void)dc_env_get_with_fallback("DISCORD_GATEWAY_URL",
                                   dotenv_paths,
                                   sizeof(dotenv_paths) / sizeof(dotenv_paths[0]),
                                   0u,
                                   &gateway_url);
...
The Fishy Discord Library provides a comprehensive C API for interacting with Discord's API v10. This reference covers all public functions, data structures, and constants available in the library. The library follows consistent patterns for error handling using `dc_status_t` return values and provides extensive utilities for JSON parsing, HTTP requests, and gateway connections.
