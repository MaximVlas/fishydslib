...
#include <stdarg.h>
    ...
    #define DC_USE_EXPLICIT_BZERO 1
    #include <strings.h>
#endif
...
#if defined(DC_USE_EXPLICIT_BZERO)
...
dc_status_t dc_string_init_from_cstr(dc_string_t* str, const char* cstr) {
    ...
    /* memcpy includes the null terminator */
...
void dc_string_free(dc_string_t* str) {
    if (str) {
        /* zeroes the memory before freeing if it contains sensitive data */
...
dc_status_t dc_string_append_buffer(dc_string_t* str, const char* data, size_t length) {
    ...
    /* Check for overflow: str->length + length + 1 */
    ...
    /* Detect if source buffer overlaps with destination buffer */
    ...
    /* Ensure capacity before copying (may reallocate and move str->data) */
    dc_status_t status = dc_string_ensure_capacity(str, new_len + 1);
    if (status != DC_OK) return status;

    /* If overlap occurred, recalculate source pointer based on new buffer address */
    if (overlap) {
        src = str->data + offset;
